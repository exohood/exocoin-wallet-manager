...
<form method="post">
    ...
    // Check for a request.
    if (!empty($_POST)) {
        // Create the Exohood wallet object.
        $exo = new Exohood\Wallet\WalletManager();

        echo "<pre id='result'>";

        switch ($_POST['action']) {
            case 'wallet_new':
                print_r($exo->wallet_from_seed());
                break;
            case 'wallet_generate':
                print_r($exo->wallet_from_seed(trim($_POST['wallet_seed'])));
                break;
            case 'wallet_mnemonic':
                print_r($exo->wallet_from_mnemonic(trim($_POST['wallet_mnemonic'])));
                break;
            case 'wallet_integrated_decode':
                print_r($exo->integrated_decode(trim($_POST['wallet_integrated_decode'])));
                break;
            case 'wallet_integrated_encode_address':
                print_r($exo->integrated_encode_address(trim($_POST['wallet_integrated_encode_address_address']), trim($_POST['wallet_integrated_encode_address_payment_id'])));
                break;
            case 'wallet_integrated_encode':
                print_r($exo->integrated_encode(trim($_POST['wallet_integrated_encode_spend']), trim($_POST['wallet_integrated_encode_view']), trim($_POST['wallet_integrated_encode_payment_id'])));
                break;
            case 'wallet_decode':
                print_r($exo->decode_address(@$_POST['wallet_decode']));
                break;
            case 'verify_privates':
                var_dump($exo->verify_privates(@$_POST['verify_privates_spend'], @$_POST['verify_privates_view']));
                break;
            case 'generate_payment_id':
                print_r($exo->generate_payment_id());
                break;
            default:
    ...
    <div>
        <strong>Generate exo Wallet From Mnemonic Words</strong>
        <code>Exohood\Wallet\WalletManager()->wallet_from_mnemonic($words)</code>
        <p>
    ...
    <div>
        ...
        <code>Exohood\Wallet\WalletManager()->decode_address($address)</code>
        <p>
    ...
    <div>
        ...
        <code>Exohood\Wallet\WalletManager()->verify_privates($spend, $view)</code>
        <p>
    ...
    <div>
        <p>
            ...
            <code>Exohood\Wallet\WalletManager()->generate_payment_id($length = 64)</code>
        </p>
